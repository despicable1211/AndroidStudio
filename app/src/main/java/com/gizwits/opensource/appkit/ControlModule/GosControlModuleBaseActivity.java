...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 */
	// 数据点"text"对应的标识名
	protected static final String KEY_TEXT = "command";
	...
	private byte cmd;
	protected static byte buttonStatus;
	protected static byte batteryLevel;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_TEXT)) {
			
					text = (byte[]) map.get(dataKey);
					//简单解析协议
					if(text[0] == -2){
						String res,tempc;
						cmd = text[4];
						if(cmd == 0x03){
							buttonStatus = text[6];
							batteryLevel = text[5];
							res = String.valueOf(buttonStatus);
							tempc = String.valueOf(batteryLevel);
							Log.d("接收到的字符串数据位电池电量为rer= ",tempc);
							Log.d("接收到的字符串数据位按键位rer= ",res);
						}
					}
				}
			}
